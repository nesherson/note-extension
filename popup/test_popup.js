...
function toggleNoContentIndicator(showIndicator) {
  ...
  if (showIndicator) noContent.classList.add("hidden");
  else noContent.classList.remove("hidden");
}
...
async function resetListOnClick() {
  ...
  while (popupBody.hasChildNodes()) popupBody.removeChild(popupBody.firstChild);
